...
//ctrl+alt+T
//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
// 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
// ssh root@45.89.66.225 (chelikon.space) suka
...
// scp ./vid/\{mickey.jpg,slava.jpg,vanya.webm,vas.webm,mickey.webm,slava.webm,vanya.jpg,vas.jpg\} root@45.89.66.225:/root/sex_for_many/public/vid
//scp ./vid/oleg.jpg root@45.89.66.225:/root/sex_for_many/public/vid
// scp root@91.217.80.183:/root/globi.sql ./vid
// scp ./vid/globi.sql root@45.89.66.225:/root
// scp -r root@91.217.80.183:/root/sex_for_many/public/vid ./public/
// scp -r root@91.217.80.183:/root/sex_for_many/public/profile ./public/

// scp root@91.217.80.183:/root/globi.sql ./
...
// ffmpeg -i slava.webm -ss 00:00:03.435 -frames:v 1 out.png
...
psql -U globi -h localhost < /root/globi.sql
===
...
const ORIGINAL = "https://chelikon.space";
...
const public_router = require('./routes/vue-router/vue-router.js');
const { meta, warnig, site_name, is_fakes_enabled } = require('./config/app.json');
//for globikon.space
...
app.use(session({ store: pg_store , cookie: { maxAge: 24 * 60 * 60 * 1000 } }, app))
...
ctx.state.is_fakes_enabled = is_fakes;
ctx.is_fakes_enabled = is_fakes;
...
ctx.btc_address = btc_address;
	...
	await pool.query(`update prim_adr set xir=$1`, [JSON.stringify(xirsys)])
		...
		try{
			...
			}else if(ctx.path == "/api/set_fakes_enabled"){
...
app.use(public_router.routes()).use(public_router.allowedMethods())
...
console.log('we got public/video');
removeDir(path.join('public','video')).then(function(d){console.log('d: ', d)}).catch(function(e){console.log(e);});
		}catch(e){
console.log(e);
try{
await mkdir('public/video/');
console.log('creating directory public/video/');
}catch(e){console.log('err in mkdir public/video: ', e)}	
...
try{
let a = await access('public/prof_video', fs.constants.F_OK);
console.log('we got public/prof_video');	
}catch(e){
try{
await mkdir('public/prof_video');
console.log('creating directory public/prof_video')
}catch(e){console.log(e)}	
}

try{
let a = await access('public/uploads/tmp', fs.constants.F_OK);
console.log('we got public/uploads/tmp');	
}catch(e){
try{
await mkdir('public/uploads/tmp');
console.log('creating directory public/uploads/tmp')
}catch(e){console.log(e)}	
}

try{
let a_1 = await access('public/images/upload/tmp', fs.constants.F_OK);
if(a_1 == undefined)console.log('we got public/images/upload/tmp: ');	
}catch(e){
try{
await mkdir('public/images/upload/tmp');
console.log('creating directory public/images/upload/tmp', e)
}catch(e){console.log(e)}	
}

try{
let b = await access('public/vid', fs.constants.F_OK);
if(b == undefined)console.log('we got directory public/vid');	
}catch(e){
try{
await mkdir('public/vid');
console.log('creating public/vid');
}catch(e){console.log(e)}	
}
var servak;
if(is_ssl_http){
...
function insert_message(msg, nick, us_id){
	...
	//let us_id = -1;
	pool.query('insert into chat(msg, us_id, nick) values($1,$2,$3)',[msg, -1, nick], function(er,r){
...
*/ 
async function send_history(ws, room_id){
	try{
		let a = await pool.query('select*from chat where us_id=$1 order by tz', [/*room_id*/-1]);
		//console.log("A: ", a)
		if(a.rows && a.rows.length > 0){
wsend(ws, { type: "history", d: a.rows })
}
		}catch(e){console.log(e);}
	}
...
if(req.url == "/gesamt") send_history(ws, -1 /*req.url.substring(1)*/)
...
if(ws.urli.substring(1) == "gesamt"){
	console.log('ws.nick: ', ws.nick);
	insert_message(l.msg, ws.nick, -1);
}
}
}
...
//pool.query('delete from vroom where nick=$1',[ws.roomname],function(er,r){
	...
	removeDir(path.join('public','video/') + d6.roomid).then(function(d){console.log('d: ',d)}).catch(function(e){console.log(e);});
	//});
		}
		if(ws.privat){
			try{
