...
function set_fakes_enabled(el){
vax("post", "/api/set_fakes_enabled", {is_enable: el.checked}, on_set_fakes, on_set_fakes_error, el, false);	
}

function on_set_fakes(l, ev){
	//alert(l.is_enable);
	//note({content: "Pay system " + l.btc_pay, type: "info", time: 5});
	let d = l.is_enable;
note({content: (d ? "true" : "false"), type: "info", time: 5});
fakes_enabled_span.textContent = l.is_enable;
ev.checked = l.is_enable;	
}
function on_set_fakes_error(l, ev){
