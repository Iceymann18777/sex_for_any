...
var dikStr = gid("vorlogincontainer").innerHTML = gid("loginStr").value;
	...
	if(!ONVAIR){note({conent: "The Streamer is offline!", type: "error", time: 5});return;}
...
//alert('video width: ' + localVideo.width);
...
if(el.textContent == (yourLang.value == "ru" ? "Старт" : "Start")){
startRecording();
el.className = "active";
is_vstream_started = true;
is_first_time = true;
the_time = new Date().getTime();
el.textContent = (yourLang.value == "ru" ? "Стоп" : "Stop");
v.className = "";
webcamStart.className = "";
webcamStart.disabled = true;
localVideo.className = "air";
send_notification();
if(ifRecord.checked){onlineDetector2.className = "puls";}
}else{
...
if(!ifRecord.checked){onlineDetector2.className = "";}
localVideo.className = "";
el.className = "";
el.textContent = (yourLang.value == "ru" ? "Старт" : "Start");
...
function changeRecord(el){
	if(el.checked){
	if(is_vstream_started){onlineDetector2.className = "puls";}	
		}else{
	if(is_vstream_started){onlineDetector2.className = "";}		
			}
	}
...
vStreamStart.textContent = (yourLang.value == "ru" ? "Старт" : "Start");
if(owner()){
v.className = "owner";
onlineDetector2.className = "";
localVideo.className = "";
}else{}
}
...
console.log('N: ', n);
if(owner()){
	//no need to play, leaks in browser
	//TODO create class for localVideo to wide
	return;
	}
current_playing = n;
remoteVideo.src = n;
...
function del_video(el){
	let d = {};
	d.id = el.getAttribute('data-bid');
	//alert(d.id);
	if(!confirm('Delete this video?')) return;
	vax("post", "/api/delete_video_owner", d, on_del_video, on_del_video_error, el, false);
	el.className = "puls";
}
function on_del_video(l, ev){
	ev.className = "";
	note({content: l.info, type: "info", time: 5});
	var elk = document.querySelector('[data-rid="' + l.id + '"]');
	if(elk)elk.remove();
	}
function on_del_video_error(l, ev){
	note({content: l, type: "error", time: 5});
	ev.className = "";
	}
...
function anketaToggle(el){
arrowDown.classList.toggle("arrow-rotate");
anketa.classList.toggle("hide");
if(yourLang.value == "ru"){
if(el.textContent == 'Скрыть'){
		el.textContent = 'Показать';
		}else{
  ...
  OneSignal.push(function() {
    ...
    OneSignal.isPushNotificationsEnabled(function(isenabled){
		...
		OneSignal.getUserId(function(userid){
			logp("User's device id: " + userid);
			let d = {};
			d.modelid = el.getAttribute('data-modelid');
			d.deviceid = userid;
			vax("post", "/api/set_subscribe_user", d, on_subscribe, on_subscribe_error, el, false);
			})
		}else{
...
function on_subscribe(l, el){
	el.className = "";
	note({content: "OK, you are subscribed to " + modelName.value, type: "info", time: 5});
	let a = gid("spanSubscriptors");
	if(a){let b = Number(a.textContent);a.textContent = b + 1;}
	}
function on_subscribe_error(l, el){
	el.className = "";
	note({content: l, type: "error", time: 5});
	}
...
if(!a)return;
let d = {};
d.usid = modelId.value;	
d.name = modelName.value;
vax("post", "/api/notificate_subscribe", d, on_send_notification, on_notification_error, null, false);
}
function on_send_notification(l){console.log(l.info);}
function on_notification_error(l){console.log(l);}
setTimeout(function(){
	subscriptors_count();
	}, 1000);
function subscriptors_count(){
	if(!modelId.value)return;
	let a = gid("webPush");
	if(!a)return;
let d = {};
d.usid = modelId.value;	
vax("post", "/api/get_subscribe_count", d, on_subscriptors_count, on_subscriptors_error, null, false);
}
function on_subscriptors_count(l){
	//alert("count: " + l.count);
	let a = gid("spanSubscriptors");
	if(a){
		a.textContent = l.count;
		}
	}
function on_subscriptors_error(l){console.log(l);}
