...
newuserdiv.innerHTML = '<div class="newuserleft"><img class="newuserfoto" src="' + (el.ava ? el.ava : '/images/default.svg') +
'"></div><div><a href="/webrtc/' + el.id + '">' + el.bname + '</a>, ' + (el.bage ? el.bage : '18') + ', ' + el.sexor + '</div>' +
(el.stat ? '<div>' + el.stat + '</div>' : '') +  
'<div>' + (is_rus() ? 'Был онлайн' : 'Last login') + ': ' + (!tolocsup() ? new Date(el.ll).toLocaleString('en-US', { day: "numeric", month: 'long', year:"numeric" }) : new Date(el.ll)) + '</div>' + '<div>' + (is_rus() ? 'Профиль создан' : 'Profile created') + ': ' + (!tolocsup() ? new Date(el.crat).toLocaleString('en-US', { day: "numeric", month: 'long', year:"numeric" }) : new Date(el.crat)) + '</div>' +
...
function tolocsup(){
	try{new Date().toLocaleString('i');}catch(e){return false;} return true;
	}
...
var pic = ev.target.pic.value;
if(pic){pic = "1";}else{pic = "0";}
var d = {ab: ab, bis: bis, bi: bi, keywort: keywort, pic: pic};
...
if(l.result.length == 0){
	note({content: "Nothing found!", type: "info", time: 5});
	return;
}	
flagi=true;
...
var b = v.lastChild.getAttribute('data-at');
if(!b)return;
var d = {ab: ab, bis: bis, bi: bi, keywort: keywort, next: b, pic: pic};
//alert(JSON.stringify(d));	
...
el.className = "";
if(!l.result)return;
if(l.result.length == 0){note({content: "Nothing found more!", type: "info", time: 5});return;}
remove_nodes(v);
formi_user_list(l.result);	
}
