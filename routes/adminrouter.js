...
adm.post("/api/set_fakes_enabled", auth, async ctx=>{
let { is_enable } = ctx.request.body;
console.log("is_enable: ", is_enable);
ctx.body = { is_enable: ctx.is_fakes_enabled }	
})
...
adm.post("/api/del_user_video", auth, async ctx=>{
	let {id} = ctx.request.body;
	if(!id) ctx.throw(400, "no id");
	let db = ctx.db;
	try{
		await db.query('delete from video where r_id=$1', [id]);
		}catch(e){ctx.throw(400, e);}
		ctx.body = {info: "ok, video deleted", id: id}
	})
...
adm.post("/api/save_post_advertise", auth, async ctx=>{
	...
	adm.post("/api/save_post_about", auth, async ctx=>{
	save_article(ctx);
	})	
...
adm.post("/send_mail", auth, async ctx=>{
	...
	t.sendMail({
		from: "root@chelikon.space",
		to: mail,
		subject:'Unsere freunschaft',
		text: `Hey Globi.Thanks for joining chelikon!
		Now you'v got your own videochat-room
		If you're a streamer and wanna get your payouts and donations in bitcoins please give to us in your videochat-room
		your bitcoin address.Attention please: on Chelikon the confirmation of the age is no need!
		 Cuz the payouts are only in bitcoins.
		 If you're a viewer and wanna give tokens to streamers you can easy buy the tokens by bitcoins 
		 Folow us.
		 Enjoy!`,// WELCOME({nick:"Globi",id:1}).text ,
		html:`<h1>Hey Globi</h1><p>Thanks for joining chelikon!<br><br>
		Now you'v got your own videochat-room<br><br>
		If you're a streamer and wanna get your payouts and donations in bitcoins please give to us in your videochat-room
		your bitcoin address.<br><br>Attention please: on chelikon the confirmation of the age is no need!
		 Cuz the payouts are only in bitcoins.<br><br>
		 If you're a viewer and wanna give tokens to streamers you can easy buy the tokens by bitcoins 
		  here.<br><br>
		 We have <a href="https://chelikon.space/basa">FAQ</a> (Frequently Asked Questions).<br><br>
		 Folow us:<br><br>
		 <br><br>Enjoy!<br><br></p> `,//WELCOME({nick:"Globi",id:1}).html
		 list:{
			 help: 'gru5@yandex.ru',
			 unsubscribe:{url:'https://chelikon.space',comment:'comment'}
			 }
	},(err,info)=>{
...
let r;
try{
r = await db.query("insert into buser(pwd,bname,email,brole, lng, promo) values('1234',$1,$2,'fake','ru',0) returning id", [ username, e ]);	
